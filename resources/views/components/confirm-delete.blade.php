<div x-show="confirmDeleteOpen" style="display: none">
    ...
    <div class="flex flex-col m-auto h-fit max-w-md bg-white fixed inset-0 shadow-md rounded-sm"
        ...
        <div class="flex justify-between items-center p-6 w-full text-xl">
            ...
            <div class="pr-2">
                <button class="font-semibold " @click="confirmDeleteOpen = false">&times;</button>
            </div>
        </div>
            ...
            <div>
                ...
                <div class="flex flex-col space-y-1">
                    <div class="flex space-x-4">
                        <p>Label: </p> <p>{{ $expense->label }}</p>
                    </div>
                    <div class="flex space-x-4">
                        <p>Amount: </p> <p> &#163;{{ $expense->formatted_amount }}</p>
                    </div>
                    <div class="flex space-x-4">
                        <p>Category: </p> <p>{{ $expense->category->name }}</p>
                    </div>
                </div>
            </div>

            <div class="flex">
                <form action="{{ route('expenses.delete', $expense) }}" method="post" class="">
                    @method('delete')
                    @csrf
                    <button type="submit"
